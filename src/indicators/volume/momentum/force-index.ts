...
export class ForceIndexIndicator extends BaseIndicator {
  ...
  private calculateForceIndex(
    ...
    const smoothedForceIndex: number[] = []
    const values: number[] = []
...
export function forceIndex(
  ...
  const config: IndicatorConfig & { smoothingPeriod?: number } = {}

  if (smoothingPeriod !== undefined) {config.smoothingPeriod = smoothingPeriod}

  const result = indicator.calculate(data, config)

  return {
    values: result.values,
    rawForceIndex: result.metadata['rawForceIndex'] as number[],
    smoothedForceIndex: result.metadata['smoothedForceIndex'] as number[]
  }
}
