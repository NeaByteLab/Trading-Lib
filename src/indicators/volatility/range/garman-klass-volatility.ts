...
export class GarmanKlassVolatilityIndicator extends VolatilityIndicator {
  ...
  private filterValidData(window: { high: number[], low: number[], open: number[], close: number[] }): Array<{ high: number, low: number, open: number, close: number }> {
    const validData: Array<{ high: number, low: number, open: number, close: number }> = []
  ...
  private calculateGarmanKlassVolatility(data: Array<{ high: number, low: number, open: number, close: number }>): number {
 ...
 */
export function garmanKlassVolatility(data: MarketData, length?: number): number[] {
