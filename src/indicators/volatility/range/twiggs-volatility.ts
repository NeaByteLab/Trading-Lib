...
export class TwiggsVolatility extends BaseIndicator {
  ...
  calculate(data: MarketData, config?: IndicatorConfig): IndicatorResult {
    ...
    const _lookback = (config?.['lookback'] as number) || 10
  ...
  private calculateTwiggsVolatility(data: MarketData, length: number, _lookback: number): number[] {
 ...
 */
export function twiggsVolatility(data: MarketData, length?: number, lookback?: number): number[] {
