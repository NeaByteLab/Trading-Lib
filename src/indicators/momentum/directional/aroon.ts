...
export class Aroon extends BaseIndicator {
  ...
  private calculateAroon(data: MarketData, length: number): {
    up: number[]
    down: number[]
  } {
  ...
  private findExtremes(data: MarketData, currentIndex: number, length: number): {
...
export function aroon(
  ...
  return {
    up: result.values,
    down: result.metadata['down'] as number[]
  }
}
