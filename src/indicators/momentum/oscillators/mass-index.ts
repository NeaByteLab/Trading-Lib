...
export class MassIndex extends BaseIndicator {
  ...
  calculate(data: MarketData | number[], config?: IndicatorConfig): IndicatorResult {
    ...
    const values = this.calculateMassIndex(data as MarketData, length)
    return {
      values,
      metadata: {
        length,
  ...
  private calculateMassIndex(data: MarketData, length: number): number[] {
    ...
    const ratio = ArrayUtils.processArray(ema1, (val1, i) => {
      const val2 = ema2[i]
      if (val1 === undefined || val2 === undefined || val2 === 0) {return 0}
      return val1 / val2
    })
    return ArrayUtils.processArray(ratio, (_, i) => {
      if (i < length * 2 - 1) {return NaN}
 ...
 */
export function massIndex(data: MarketData, length?: number): number[] {
