 ...
 */
function calculateElderRayIndex(data: MarketData, length: number = 13): {
  bullPower: number[]
  bearPower: number[]
} {
  // Validate input data
...
export class ElderRayIndex extends BaseIndicator {
  ...
  calculate(data: MarketData | number[], config?: IndicatorConfig): IndicatorResult {
    ...
    const { bullPower, bearPower } = calculateElderRayIndex(data as MarketData, length)
    return {
      values: bullPower,
      metadata: {
        length,
 ...
 * @returns Elder Ray Index values
 */
export function elderRayIndex(data: MarketData, length: number = 13): {
  bullPower: number[]
  bearPower: number[]
} {
  ...
  return {
    ...
    bearPower: result.metadata['bearPower'] as number[]
  }
}
