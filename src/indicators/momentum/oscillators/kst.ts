...
export class KST extends BaseIndicator {
  ...
  calculate(data: MarketData | number[], config?: IndicatorConfig): IndicatorResult {
    ...
    const roc1 = pineLength(config?.['roc1'] as number || 10, 10)
    const roc2 = pineLength(config?.['roc2'] as number || 15, 15)
    const roc3 = pineLength(config?.['roc3'] as number || 20, 20)
    const roc4 = pineLength(config?.['roc4'] as number || 30, 30)
    const sma1 = pineLength(config?.['sma1'] as number || 10, 10)
    const sma2 = pineLength(config?.['sma2'] as number || 10, 10)
    const sma3 = pineLength(config?.['sma3'] as number || 10, 10)
    const sma4 = pineLength(config?.['sma4'] as number || 15, 15)
