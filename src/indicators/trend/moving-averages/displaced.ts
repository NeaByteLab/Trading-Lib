 ...
 */
function calculateDisplacedMA(data: number[], length: number = 20, displacement: number = 0, maType: 'sma' | 'ema' | 'wma' | 'hull'): number[] {
  if (data.length === 0) {
    return []
  }
...
export class DisplacedMovingAverage extends BaseIndicator {
  ...
  calculate(data: MarketData | number[], config?: IndicatorConfig): IndicatorResult {
    ...
    const displacement = config?.['displacement'] as number || 0
    const maType = (config?.['maType'] as string) || 'sma'
    const values = calculateDisplacedMA(source, length, displacement, maType as 'sma' | 'ema' | 'wma' | 'hull')
    return {
      values,
      metadata: {
        length,
 ...
 * @returns Displaced MA values array
 */
export function displaced(data: MarketData | number[], length?: number, displacement?: number, maType?: 'sma' | 'ema' | 'wma' | 'hull', source?: string): number[] {
  const config: Partial<IndicatorConfig> = {}
  if (displacement !== undefined) {config['displacement'] = displacement}
  if (maType !== undefined) {config['maType'] = maType}
