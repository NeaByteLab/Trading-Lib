...
export class KAMA extends BaseIndicator {
  ...
  calculate(data: MarketData | number[], config?: IndicatorConfig): IndicatorResult {
    ...
    const fastPeriod = config?.['fastPeriod'] as number || 2
    const slowPeriod = config?.['slowPeriod'] as number || 30
    const values = calculateKAMA(source, length, fastPeriod as number, slowPeriod as number)
    return {
      values,
      metadata: {
        length,
 ...
 */
export function kama(data: MarketData | number[], length?: number, fastPeriod?: number, slowPeriod?: number, source?: string): number[] {
