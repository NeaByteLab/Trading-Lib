...
export class RainbowIndicator extends BaseIndicator {
  ...
  calculate(data: MarketData | number[], config?: IndicatorConfig & { periods?: number[] }): IndicatorResult {
    ...
    return {
      ...
      metadata: {
        ...
        bands: bands as unknown as number[]
      }
    }
  }
}

export function rainbow(data: MarketData | number[], periods?: number[], source?: string): number[][] {
  const indicator = new RainbowIndicator()
  const config: IndicatorConfig & { periods?: number[] } = {}

  if (periods !== undefined) {config.periods = periods}
  if (source !== undefined) {config.source = source}

  const result = indicator.calculate(data, config)
  return result.metadata['bands'] as unknown as number[][]
}
