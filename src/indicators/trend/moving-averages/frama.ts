...
export class FRAMAIndicator extends BaseIndicator {
   ...
   */
  private calculateFRAMA(data: number[], length: number, fc: number, sc: number): number[] {
    const frama: number[] = []
   ...
   */
  private calculateFractalDimension(data: number[], currentIndex: number, length: number): number {
   ...
   */
  private calculateWindowStats(window: number[]): { low: number, range: number } {
   ...
   */
  private calculateBoxCounts(window: number[], low: number, range: number): number[] {
   ...
   */
  private countBoxesForScale(window: number[], low: number, boxSize: number): number {
    let boxCount = 0
   ...
   */
  private calculateFractalDimensionFromBoxCounts(boxCounts: number[]): number {
    const scales = [1, 2, 4, 8]
   ...
   */
  private calculateLinearRegression(x: number[], y: number[]): { slope: number } {
    const n = x.length
   ...
   */
  private calculateAdaptiveAlpha(fractalDimension: number, fc: number, sc: number): number {
