 ...
 */
function calculateZLEMA(data: number[], length: number = 20): number[] {
...
export class ZLEMA extends BaseIndicator {
  ...
  calculate(data: MarketData | number[], config?: IndicatorConfig): IndicatorResult {
    ...
    if (Array.isArray(data)) {
      ...
      priceData = data[source as keyof MarketData] as number[]
    }
 ...
 * @returns Array of ZLEMA values
 */
export function zlema(data: number[] | MarketData, length: number = 20): number[] {
