...
export class FibonacciRetracementsIndicator extends BaseIndicator {
  ...
  private calculateRetracements(
    ...
    const retracements: number[] = []
    const level0: number[] = []
    const level236: number[] = []
    const level382: number[] = []
    const level500: number[] = []
    const level618: number[] = []
    const level786: number[] = []
    const level100: number[] = []
   ...
   */
  private calculateRetracementLevels(highPrice: number, lowPrice: number, range: number, currentPrice: number) {
    ...
    const currentLevel = this.determineRetracementLevel(
      currentPrice, level0Val, level236Val, level382Val, level500Val, level618Val, level786Val
    )
  ...
  private determineRetracementLevel(
    ...
    if (currentPrice >= level0) {return 0}
    if (currentPrice >= level236) {return 0.236}
    if (currentPrice >= level382) {return 0.382}
    if (currentPrice >= level500) {return 0.5}
    if (currentPrice >= level618) {return 0.618}
    if (currentPrice >= level786) {return 0.786}
    return 1
  }
...
export function fibonacciRetracements(
  ...
  if (swingPeriod !== undefined) {config.swingPeriod = swingPeriod}
  ...
  return {
    ...
    level0: result.metadata['level0'] as number[],
    level236: result.metadata['level236'] as number[],
    level382: result.metadata['level382'] as number[],
    level500: result.metadata['level500'] as number[],
    level618: result.metadata['level618'] as number[],
    level786: result.metadata['level786'] as number[],
    level100: result.metadata['level100'] as number[]
  }
}
