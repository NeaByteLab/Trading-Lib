...
export class LinearRegressionIndicator extends BaseIndicator {
  ...
  private calculateLinearRegression(values: number[]): LinearRegressionResult {
 ...
 */
export function linearRegression(data: MarketData | number[], length?: number, source?: string): {
  forecast: number[]
  slope: number[]
  intercept: number[]
  rSquared: number[]
  ...
  return {
    ...
    slope: result.metadata['slope'] as number[],
    intercept: result.metadata['intercept'] as number[],
    rSquared: result.metadata['rSquared'] as number[],
    upperBand: result.metadata['upperBand'] as number[],
    lowerBand: result.metadata['lowerBand'] as number[]
  }
}
