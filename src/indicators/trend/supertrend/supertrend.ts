...
export class SuperTrend extends BaseIndicator {
  ...
  private calculateSuperTrend(data: MarketData, period: number, multiplier: number): { superTrend: number[], direction: number[] } {
    ...
    const results = ArrayUtils.processArray(data.close, (_, i) => {
      ...
      const { finalUpper, finalLower } = this.calculateFinalBands(
        basicUpper, basicLower, prevSuperTrend, prevSuperTrend, i, period
      )
      const { superTrendValue, currentDirection } = this.calculateSuperTrendValue(
   ...
   */
  private calculateBasicBands(high: number, low: number, atrValue: number, multiplier: number): {
    basicUpper: number
    basicLower: number
  } {
