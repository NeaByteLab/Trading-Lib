...
export class HarmonicPatternsIndicator extends BaseIndicator {
  ...
  calculate(data: MarketData | number[], config?: IndicatorConfig): IndicatorResult {
    ...
    const xabLevels: number[] = []
    const abcLevels: number[] = []
    const bcdLevels: number[] = []
    const xadLevels: number[] = []
  ...
  private detectHarmonicPattern(data: MarketData, currentIndex: number, lookback: number): HarmonicPatternResult {
  ...
  private findSwingPoints(highs: number[], lows: number[]): Array<{ price: number; type: 'high' | 'low'; index: number }> {
    const swings: Array<{ price: number; type: 'high' | 'low'; index: number }> = []
  ...
  private calculateFibonacciRatios(swings: Array<{ price: number; type: 'high' | 'low'; index: number }>): {
    xab: number
    abc: number
    bcd: number
    xad: number
  ...
  private checkBatPattern(ratios: { xab: number; abc: number; bcd: number; xad: number }): HarmonicPatternResult {
    const tolerance = 0.1
    const isBat =
      Math.abs(ratios.xab - 0.382) < tolerance &&
  ...
  private checkButterflyPattern(ratios: { xab: number; abc: number; bcd: number; xad: number }): HarmonicPatternResult {
    const tolerance = 0.1
    const isButterfly =
      Math.abs(ratios.xab - 0.786) < tolerance &&
      Math.abs(ratios.abc - 0.382) < tolerance &&
  ...
  private checkCrabPattern(ratios: { xab: number; abc: number; bcd: number; xad: number }): HarmonicPatternResult {
    const tolerance = 0.1
    const isCrab =
      Math.abs(ratios.xab - 0.382) < tolerance &&
  ...
  private checkCypherPattern(ratios: { xab: number; abc: number; bcd: number; xad: number }): HarmonicPatternResult {
    const tolerance = 0.1
    const isCypher =
      Math.abs(ratios.xab - 0.382) < tolerance &&
  ...
  private checkGartleyPattern(ratios: { xab: number; abc: number; bcd: number; xad: number }): HarmonicPatternResult {
    const tolerance = 0.1
    const isGartley =
      Math.abs(ratios.xab - 0.618) < tolerance &&
      Math.abs(ratios.abc - 0.382) < tolerance &&
  ...
  private checkSharkPattern(ratios: { xab: number; abc: number; bcd: number; xad: number }): HarmonicPatternResult {
    const tolerance = 0.1
    const isShark =
      Math.abs(ratios.xab - 1.13) < tolerance &&
 ...
 */
export function harmonicPatterns(data: MarketData, length?: number): {
  ...
  return {
    patterns: (result.metadata['patternTypes'] as string).split(',').map(p => p as HarmonicPattern),
    confidences: result.values,
    levels: {
      xab: result.metadata['xabLevels'] as number[],
      abc: result.metadata['abcLevels'] as number[],
      bcd: result.metadata['bcdLevels'] as number[],
      xad: result.metadata['xadLevels'] as number[]
    }
  }
}
