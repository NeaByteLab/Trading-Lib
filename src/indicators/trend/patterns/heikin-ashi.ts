 ...
 */
export function heikinAshi(data: MarketData): {
  open: number[]
  high: number[]
  low: number[]
  close: number[]
  ...
  return {
    open: result.metadata['open'] as number[],
    high: result.metadata['high'] as number[],
    low: result.metadata['low'] as number[],
    close: result.metadata['close'] as number[]
  }
}
