...
export const LargeDatasetCalculations = {
   ...
   */
  calculateRSIChunked(data: number[], length: number, chunkSize: number = 10000): number[] {
   ...
   */
  calculateRollingStatisticChunked(
    data: number[],
    windowSize: number,
 ...
 */
export function vectorizedProcess<T>(data: number[], operation: (val: number, index: number) => T): T[] {
 ...
 * @returns Processed array
 */
export function optimizedWindowProcess<T>(
  data: number[],
  windowSize: number,
  operation: (window: number[], startIndex: number) => T
): T[] {
  const result: T[] = []
  const blockSize = Math.min(windowSize * 2, 128)
  for (let i = 0; i < data.length; i++) {
    if (i < windowSize - 1) {
      result.push(operation([], i) as T)
      continue
    }
    const window: number[] = []
