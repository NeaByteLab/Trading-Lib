 ...
 */
export function calculateKNNClassifier(data: MarketData, k: number = 5, lookback: number = 20, features: number = 5): number[] {
 ...
 */
function extractFeatures(close: number[], high: number[], low: number[], volume: number[], index: number, features: number): number[] {
 ...
 */
function findNearestNeighbors(close: number[], currentIndex: number, lookback: number, currentFeatures: number[], k: number): number[] {
 ...
 */
function calculateEuclideanDistance(features1: number[], features2: number[]): number {
 ...
 */
function classifyFromNeighbors(neighbors: number[]): number {
  if (neighbors.length === 0) {return 0.5}
  const validNeighbors = neighbors.filter(n => !isNaN(n) && isFinite(n))
  if (validNeighbors.length === 0) {return 0.5}
