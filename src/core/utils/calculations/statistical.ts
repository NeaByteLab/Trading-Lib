 ...
 */
export function kthSmallest(array: number[], k: number): number {
 ...
 * @throws {Error} If array is empty or k is out of bounds
 */
export function kthLargest(array: number[], k: number): number {
  if (!Array.isArray(array)) {
    throw new Error(ERROR_MESSAGES.ARRAY_REQUIRED)
  }
  const validValues = sanitizeArray(array)
 ...
 */
export function calculatePercentile(array: number[], percentile: number): number {
  if (!Array.isArray(array)) {
    throw new Error(ERROR_MESSAGES.ARRAY_REQUIRED)
  }
 ...
 */
export function calculateShannonEntropy(data: number[], length: number, bins: number = 8): number[] {
