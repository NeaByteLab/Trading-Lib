 ...
 * @throws {Error} If data is empty or missing required fields
 */
export function calculatePriceVolumeTrend(close, volume) {
    if (!close || close.length === 0) {
        throw new Error(ERROR_MESSAGES.EMPTY_DATA);
    }
    if (!volume || volume.length === 0) {
 ...
 * @throws {Error} If data is empty or missing required fields
 */
export function calculatePositiveVolumeIndex(close, volume) {
    if (!close || close.length === 0) {
        throw new Error(ERROR_MESSAGES.EMPTY_DATA);
    }
    if (!volume || volume.length === 0) {
 ...
 * @throws {Error} If data is empty or missing required fields
 */
export function calculateTwiggsMoneyFlow(high, low, close, volume) {
    if (!close || close.length === 0) {
        throw new Error(ERROR_MESSAGES.EMPTY_DATA);
    }
    if (!volume || volume.length === 0) {
 ...
 * @throws {Error} If data is empty or missing required fields
 */
export function calculateAmihudIlliquidity(close, volume) {
    if (!close || close.length === 0) {
        throw new Error(ERROR_MESSAGES.EMPTY_DATA);
    }
    if (!volume || volume.length === 0) {
 ...
 */
export function calculateCumulativeVolume(volume, startIndex, endIndex) {
 ...
 */
export function calculateAverageVolume(volume) {
    if (!volume || volume.length === 0) {
        return 1;
    }
 ...
 */
export function calculateVolumeRatio(volume, avgVolume) {
    if (avgVolume === 0) {
        return 1;
    }
 ...
 */
export function calculateVolumeRateOfChange(volume, length = 14) {
    if (!volume || volume.length === 0) {
        throw new Error(ERROR_MESSAGES.EMPTY_DATA);
    }
    if (length <= 0) {
 ...
 */
export function calculateVWMA(prices, volume, length = 20) {
 ...
 * @throws {Error} If data is empty or arrays have different lengths
 */
export function calculateVAMA(prices, volume, length = 20, volumeThreshold = 0) {
    if (!prices || prices.length === 0) {
        throw new Error(ERROR_MESSAGES.EMPTY_DATA);
    }
    if (!volume || volume.length === 0) {
 ...
 * @throws {Error} If data is empty or arrays have different lengths
 */
export function calculateVWSMA(prices, volume, length = 20) {
    if (!prices || prices.length === 0) {
        throw new Error(ERROR_MESSAGES.EMPTY_DATA);
    }
    if (!volume || volume.length === 0) {
 ...
 * @throws {Error} If data is empty or missing required fields
 */
export function calculateKylesLambda(close, volume, period = 20) {
    if (!close || close.length === 0) {
        throw new Error(ERROR_MESSAGES.EMPTY_DATA);
    }
    if (!volume || volume.length === 0) {
 ...
 */
function calculateSignedSqrtVolume(returns, volume) {
    const signedVolume = [];
 ...
 * @throws {Error} If data is empty or arrays have different lengths
 */
export function calculateVortexIndicator(high, low, close, length = 14) {
 ...
 */
function calculatePriceChange(windowClose, index) {
    if (index === 0) {
        return 0;
    }
 ...
 */
function processVolumeForBucket(vol, priceChange, currentBucketVolume, bucketSize) {
    let buyVolume = 0;
    let sellVolume = 0;
 ...
 */
function calculateBucketVolumes(windowClose, windowVolume, bucketSize) {
    let buyVolume = 0;
    let sellVolume = 0;
 ...
 * @returns VPIN value
 */
function calculateVPINForWindow(windowClose, windowVolume, buckets) {
 ...
 */
export function calculateVPIN(close, volume, length = 50, buckets = 50) {
 ...
 * @throws {Error} If data is empty or arrays have different lengths
 */
export function calculateVolumeProfile(prices, volume, levels = 10) {
    if (!prices || prices.length === 0) {
        throw new Error(ERROR_MESSAGES.EMPTY_DATA);
    }
    if (!volume || volume.length === 0) {
