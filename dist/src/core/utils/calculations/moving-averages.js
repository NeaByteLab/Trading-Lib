 ...
 */
export function calculateOptimizedMovingAverage(data, length, type) {
 ...
 */
function calculateOptimizedSMA(data, length) {
    const result = [];
 ...
 */
function calculateOptimizedEMA(data, length) {
    if (data.length === 0) {
        return [];
    }
 ...
 */
function calculateOptimizedWMA(data, length) {
    if (data.length === 0 || length <= 0) {
        return [];
    }
 ...
 * @throws {Error} If data is empty or length is invalid
 */
export function calculateKAMA(data, length, fastPeriod = 2, slowPeriod = 30) {
    if (!data || data.length === 0) {
        throw new Error(ERROR_MESSAGES.EMPTY_DATA);
    }
    if (length <= 0) {
 ...
 */
function calculateEfficiencyRatio(data, index, length) {
    if (index < length) {
        return 0;
    }
 ...
 * @returns CMO value
 */
function calculateCMOForVIDYA(changes, currentIndex, cmoLength) {
    let gains = 0;
    let losses = 0;
    let validCount = 0;
 ...
 * @returns VIDYA value
 */
function calculateVIDYAValue(currentValue, previousVIDYA, length, cmo) {
 ...
 * @throws {Error} If data is empty or length is invalid
 */
export function calculateVIDYA(data, length = 14, cmoLength = 9) {
    if (!data || data.length === 0) {
        throw new Error(ERROR_MESSAGES.EMPTY_DATA);
    }
