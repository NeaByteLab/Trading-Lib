 ...
 */
export function calculateStochasticRSI(rsiValues, kLength, dLength) {
 ...
 * @throws {Error} If data is empty or length is invalid
 */
export function calculateCMO(data, length) {
    if (!data || data.length === 0) {
        throw new Error(ERROR_MESSAGES.EMPTY_DATA);
    }
    if (length <= 0) {
 ...
 */
export function calculateKlingerOscillator(data, shortPeriod = 34, longPeriod = 55) {
 ...
 */
function calculateSumVolume(close, high, low, volume) {
    const sv = [];
    let trend = 0;
    let prevSum = 0;
 ...
 */
function calculateSum(values, currentIndex, period) {
    let sum = 0;
 ...
 */
export function calculateKST(data, roc1 = 10, roc2 = 15, roc3 = 20, roc4 = 30, sma1 = 10, sma2 = 10, sma3 = 10, sma4 = 15) {
    if (!data || data.length === 0) {
        throw new Error(ERROR_MESSAGES.EMPTY_DATA);
    }
