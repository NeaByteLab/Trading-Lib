 ...
 */
function calculateZigZag(data, deviation = 5, depth = 12) {
    const { high, low } = data;
    if (high.length === 0 || low.length === 0) {
        return { values: [], swings: [] };
    }
    const swings = [];
    const values = new Array(high.length).fill(NaN);
 ...
 */
function findLocalMaximum(data, centerIndex, windowSize) {
 ...
 */
function findLocalMinimum(data, centerIndex, windowSize) {
    const start = Math.max(0, centerIndex - windowSize);
    const end = Math.min(data.length - 1, centerIndex + windowSize);
    let minIndex = start;
 ...
 */
function findNextSwingPoint(high, low, currentPoint, deviation, depth) {
...
export class ZigZag extends BaseIndicator {
    ...
    validateInput(data, config) {
        ...
        const deviation = config?.['deviation'] || 5;
        const depth = config?.['depth'] || 12;
    ...
    calculate(data, config) {
        ...
        const deviation = config?.['deviation'] || 5;
        const depth = config?.['depth'] || 12;
        const { values, swings } = calculateZigZag(data, deviation, depth);
        return {
            values,
            metadata: {
 ...
 * @returns Object with zigzag values and swing points
 */
export function zigZag(data, deviation = 5, depth = 12) {
    const indicator = new ZigZag();
    const config = { deviation, depth };
    const result = indicator.calculate(data, config);
    return {
        values: result.values,
        swings: result.metadata['swings']
    };
}
