 ...
 */
function calculateBasicBands(high, low, atrValue, multiplier) {
 ...
 */
function calculateFinalBands(basicUpper, basicLower, prevFinalUpper, prevFinalLower, index, period) {
    if (index === period) {
 ...
 */
function calculateSuperTrendValue(prevSuperTrend, finalUpper, finalLower, currentClose, prevDirection) {
...
function calculateSuperTrend(data, period, multiplier) {
    ...
    const results = ArrayUtils.processArray(data.close, (_, i) => {
        ...
        const { finalUpper, finalLower } = calculateFinalBands(basicUpper, basicLower, prevSuperTrend, prevSuperTrend, i, period);
        const { superTrendValue, currentDirection } = calculateSuperTrendValue(prevSuperTrend, finalUpper, finalLower, close, prevDirection);
