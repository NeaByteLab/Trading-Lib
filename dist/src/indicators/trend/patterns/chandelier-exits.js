 ...
 */
function updateTrailingStops(longExitLevel, shortExitLevel, prevLongExit, prevShortExit) {
 ...
 */
function calculateChandelierExits(data, atrPeriod = 14, multiplier = 3) {
    ...
    for (let i = 0; i < data.close.length; i++) {
        ...
        const { longExit: finalLongExit, shortExit: finalShortExit } = updateTrailingStops(longExitLevel, shortExitLevel, prevLongExit, prevShortExit);
...
export class ChandelierExits extends BaseIndicator {
    ...
    calculate(data, config) {
        ...
        const atrPeriod = config?.['atrPeriod'] || 14;
        const multiplier = config?.['multiplier'] || 3;
        const { longExit, shortExit } = calculateChandelierExits(data, atrPeriod, multiplier);
        return {
            values: longExit,
            metadata: {
 ...
 * @returns Object with long and short exit levels
 */
export function chandelierExits(data, atrPeriod = 14, multiplier = 3) {
    ...
    return {
        ...
        shortExit: result.metadata['shortExit']
    };
}
