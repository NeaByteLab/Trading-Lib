...
export class MarketRegimeClassifier extends BaseIndicator {
    ...
    calculate(data, config) {
        ...
        const values = this.calculateRegimeClassification(data, length, threshold);
        return {
            values,
            metadata: {
                length,
    ...
    calculateRegimeClassification(data, length, threshold) {
        ...
        return ArrayUtils.processArray(data.close, (_, i) => {
            ...
            if (volScore === 1 && atrScore === 1) {
                return 2;
            }
            if (trendScore === 1 && volScore === 0) {
                return 1;
            }
            if (atrScore === 1 && trendScore === 0) {
                return 3;
            }
            return 0;
        });
 ...
 */
export function marketRegimeClassifier(data, length, threshold) {
