 ...
 */
function calculateChaikinOscillator(data, fastLength = 3, slowLength = 10) {
...
export class ChaikinOscillator extends BaseIndicator {
    ...
    validateInput(data, config) {
        ...
        const fastLength = config?.['fastLength'] || 3;
        const slowLength = config?.['slowLength'] || 10;
    ...
    calculate(data, config) {
        ...
        const fastLength = config?.['fastLength'] || 3;
        const slowLength = config?.['slowLength'] || 10;
        const values = calculateChaikinOscillator(data, fastLength, slowLength);
        return {
            values,
            metadata: {
                length: fastLength,
 ...
 * @returns Chaikin Oscillator values array
 */
export function chaikinOscillator(data, fastLength = 3, slowLength = 10) {
