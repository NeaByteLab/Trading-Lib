 ...
 */
function calculateRogersSatchell(high, low, open, close) {
    return ArrayUtils.processArray(close, (_, i) => {
        ...
        return MathUtils.sqrt((h - c) * (h - o) + (l - c) * (l - o));
    });
}
/**
 ...
 */
function calculateGarmanKlass(high, low, open, close) {
    return ArrayUtils.processArray(close, (_, i) => {
        const h = high[i];
        const l = low[i];
        const o = open[i];
 ...
 */
function calculateParkinson(high, low) {
 ...
 */
function calculateYangZhangVolatility(data, length = 20) {
    ...
    return ArrayUtils.processArray(data.close, (_, i) => {
        ...
        return MathUtils.sqrt(overnightVar + k * parkinsonVar + (1 - k) * (rsVar + gkVar) / 2);
    });
}
/**
...
export class YangZhangVolatility extends BaseIndicator {
    ...
    calculate(data, config) {
        ...
        const length = config?.['length'] || 20;
        const values = calculateYangZhangVolatility(data, length);
        return {
            values,
            metadata: {
 ...
 * @returns Array of Yang-Zhang volatility values
 */
export function yangZhangVolatility(data, length = 20) {
