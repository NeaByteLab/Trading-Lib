...
describe('Accumulation Distribution', () => {
  let testData: { open: number[], high: number[], low: number[], close: number[], volume: number[] }
  let marketData: any
  ...
  describe('Edge Cases', () => {
    ...
    it('should throw error for missing high data', () => {
      expect(() => accumulationDistribution({ open: [1], low: [1], close: [1], volume: [1] } as any)).toThrow(ERROR_MESSAGES.MISSING_OHLCV)
    })

    it('should throw error for missing low data', () => {
      expect(() => accumulationDistribution({ open: [1], high: [1], close: [1], volume: [1] } as any)).toThrow(ERROR_MESSAGES.MISSING_OHLCV)
    })

    it('should throw error for missing close data', () => {
      expect(() => accumulationDistribution({ open: [1], high: [1], low: [1], volume: [1] } as any)).toThrow(ERROR_MESSAGES.MISSING_OHLCV)
    })

    it('should throw error for missing volume data', () => {
      expect(() => accumulationDistribution({ open: [1], high: [1], low: [1], close: [1] } as any)).toThrow(ERROR_MESSAGES.MISSING_VOLUME)
    })
